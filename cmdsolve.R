...
varClust <- function(cor.sp,k) {
  ...
  clust2 <- kmeansvar(cor.sp, init =  P3$cluster)
  clust2$cluster
}
...
# neuer sim.structure-Befehl bei dem man die uniquenesses als Matrix eingeben kann (mit Residualkorrelationen)

sim.structure.stella <- function (fx = NULL, Phi = NULL, fy = NULL, f = NULL, n = 0, 
                                  ...
                                  d = NULL, cat = 5, mu = 0) 
{
  ...
  if (n > 0) {
    mu <- rep(mu, nvar)
    observed <- mvrnorm(n = n, mu, Sigma = model, tol = 1e-06, 
                        empirical = FALSE)
    if (items) {
